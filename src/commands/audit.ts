...
export function runAudit(projectRoot: string): AuditResult {
  ...
  for (const entry of plans) {
    const rel = path.relative(projectRoot, entry.filePath);
    if (!entry.hasFrontmatter && config.validation.require_plan_frontmatter) {
      issues.push({
        severity: 'warning',
  ...
  for (const entry of reviews) {
    const rel = path.relative(projectRoot, entry.filePath);
    if (!entry.hasFrontmatter && config.validation.require_review_frontmatter) {
      issues.push({
        severity: 'warning',
 ...
 */
function buildDocHealthFromAudit(result: AuditResult, generatedAt: string): string {
...
export function runAuditCommand(options: AuditOptions): void {
  ...
  if (options.write) {
    ...
    if (config) {
      ...
      const content = buildDocHealthFromAudit(result, generatedAt);
